...
- [x] **A11y-4. Form errors don't auto-focus the invalid field** — added `useRef` + `ref.current?.focus()` in every error branch, plus `aria-invalid={!!error}`, `aria-describedby`, and `role="alert"` error elements across HomeScene (pitch textarea), LoginScene (email input), and Comments (new-comment + edit textareas).
...
- [x] **Perf-2. Frame sequences total 6.1 MB** — Converted all three frame sequences (frames-1, 2, 3) to AVIF via `scripts/convert-frames-to-avif.mjs`. Realized savings: frames-1 1.76MB→0.77MB (-56%), frames-2 3.98MB→2.10MB (-47%), frames-3 4.44MB→2.37MB (-47%). The JPEGs were deleted; `HeroPanel.tsx` canvas loader points at `.avif`. Bonus: also AVIF'd the four scene background PNGs (firstimage, transition-poster, raking, winbuiltapp) — saved another ~25MB. fps stayed at 18 (visually smoother; deferred); srcset deferred (phones see at-rest only, the AT-rest is now ~35KB AVIF, no need for half-res).
...
- [x] **P-2. The "edited" indicator threshold (>1s diff)** — Migration `014_comments_is_edited.sql` shipped in commit `5fc3006`: adds `is_edited boolean` column + before-update trigger that flips it true ONLY when `body IS DISTINCT FROM` previous value. Comments.tsx now reads `comment.is_edited` directly — the timestamp-diff hack is gone. False-positive edited badges from future schema work that touches `updated_at` are no longer possible.
